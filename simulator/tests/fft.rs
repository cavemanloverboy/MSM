...
fn test_arrayfire_1_d_inplace_c32_integration() {
    ...
    forward_inplace::<T>(&mut array, num::FromPrimitive::from_usize(K).unwrap(), S)
    ...
    inverse_inplace::<T>(&mut array, num::FromPrimitive::from_usize(K).unwrap(), S)
...
fn test_arrayfire_1_d_inplace_c64_integration() {
    ...
    use approx::assert_abs_diff_eq;
    use arrayfire::{Array, Dim4};
    use msm_simulator::utils::fft::{forward_inplace, inverse_inplace};
    use num::Complex;
...
fn test_arrayfire_1_d_c32_integration() {
    ...
    use approx::assert_abs_diff_eq;
    use arrayfire::{Array, Dim4};
    use msm_simulator::utils::fft::{forward, inverse};
    use num::Complex;
    ...
    // Perform inplace FFT + inverse
    let output = forward::<T>(&array, num::FromPrimitive::from_usize(K).unwrap(), S)
        .expect("forward fft failed");
    let output = inverse::<T>(&output, num::FromPrimitive::from_usize(K).unwrap(), S)
        .expect("inverse fft failed");
