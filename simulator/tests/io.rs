...
fn test_io() {
    ...
    npz.add_array("host", &host).unwrap();
    npz.finish().unwrap();

    // Write to npy
    npy::to_file("data.npy", host.clone()).unwrap();
    ...
    assert_eq!(data.to_vec(), host.into_raw_vec());

    std::fs::remove_file("data.npy").unwrap();
}
