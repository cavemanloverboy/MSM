use ndarray::{array, arr2, s, Array, ShapeBuilder};
...
fn test_2_d_fft_with_ndarray() {
    ...
    let mut my_array: [[Complex::<T>; FFT_SIZE]; FFT_SIZE] = [[Complex::<T> { re: 1.0, im: -1.0 }; FFT_SIZE]; FFT_SIZE];
    ...
    // Forward FFT
    {
        ...
        println!("{}", "-".repeat(40));
        transpose::transpose(buffer.clone().as_slice().unwrap(), buffer.as_slice_mut().unwrap(), FFT_SIZE, FFT_SIZE);
