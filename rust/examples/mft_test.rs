...
fn main() {
     ...
     let particle_mass: f64 = total_mass / 1e11;//(1e-22)*8.97e-67; // ev/c^2 * ev/c^2 to solar masses
     let sim_name: String = format!("mft-test-1e10-{}", S);
     let total_sim_time: T = 100.0 / (msm::constants::POIS_CONST * total_mass / (axis_length as f64).powf(3.0)).sqrt() as T;
     ...
     // Gaussian Parameters
     let kmax = msm::utils::fft::get_kgrid::<T,S>(axis_length / 128.0 as f32)
          .iter()
