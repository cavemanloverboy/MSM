...
fn main() {
     ...
     for stream in 0..num_streams {
          ...
          let total_mass: f64 = 1e10;
          let particle_mass: f64 = total_mass / 1e10;//(1e-22)*8.97e-67; // ev/c^2 * ev/c^2 to solar masses
          let sim_name: String = format!("cold-gauss-{:05}", stream);
          ...
          println!("dτ = {:e}, χ = {:e}, ΔT = {:e}, N = {}", dτ, χ, total_sim_time, total_mass / particle_mass);
          ...
          // Gaussian Parameters
          let kmax = msm::utils::fft::get_kgrid::<T,S>(axis_length / S as T)
               .iter()
